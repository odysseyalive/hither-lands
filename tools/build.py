...
def main():
    ...
    #
    # FAangband (like mainline Angband 4.2) has NO runtime player gender, so the
    # $GENDER prf variable can't resolve. Gender is therefore an INSTALL-TIME
    # choice: we emit a male and a female variant of the player-tile prf keyed on
    # $RACE / $CLASS only, plus a default (xtra-<suffix>.prf == male). install.sh
    # selects one via `--gender` by overwriting the default. Switching gender is a
    # re-install, no rebuild.
    if pvars:
        xtra_pref = m["tileset"]["pref"].replace("graf-", "xtra-", 1)
        stem = xtra_pref[:-len(".prf")] if xtra_pref.endswith(".prf") else xtra_pref

        def emit_player_xtra(target_gender):
            """One xtra file: the chosen gender's tile per race/class group.
            Class-only groups are emitted before race+class so the more specific
            race+class line wins (prf = last match wins)."""
            groups = {}
            for pv in pvars:
            ...
            lines = [
                f"# File: {stem}-{target_gender.lower()}.prf",
                "#",
                "# Generated by tools/build.py from manifest.json -- do not hand-edit.",
                "#",
                f"# Player sprites for {target_gender} characters, keyed on "
                "$RACE / $CLASS (gender chosen at install time).",
                "",
            ]
            for key in sorted(groups, key=lambda k: (1 if k[0] else 0, k[0],
                                                     1 if k[1] else 0, k[1])):
                race, cls = key
                choices = groups[key]
                pv = (choices.get(target_gender) or choices.get("Male")
                ...
                lines.append(f"monster:<player>:0x{attr:02X}:0x{char:02X}")
            return "\n".join(lines) + "\n"
        ...
        prf += ["", "# Player variant sprites (race/class; gender set at install):",
                f"%:{xtra_pref}"]
